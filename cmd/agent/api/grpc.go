...
// and streams them to clients as StreamTagsResponse events.
func (s *serverSecure) StreamTags(in *pb.StreamTagsRequest, out pb.AgentSecure_StreamTagsServer) error {
	...
	// NOTE: Subscribe returns the current entities alongside a channel for
	// all upcoming events. This is to avoid having to completely drain the
	// channel from existing entities before being able to process any
	// upcoming events, improving overall Tagger throughput.
